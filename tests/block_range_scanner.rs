...
async fn live_mode_processes_all_blocks_respecting_block_confirmations() -> anyhow::Result<()> {
    ...
    let mut brs = BlockRangeScanner::new().connect(provider.clone()).await?;
...
async fn stream_from_starts_at_latest_once_it_has_enough_confirmations() -> anyhow::Result<()> {
    ...
    let brs = BlockRangeScanner::new().connect(provider.clone()).await?;
...
async fn historical_emits_correction_range_when_reorg_below_end() -> anyhow::Result<()> {
    ...
    let mut brs = BlockRangeScanner::new().max_block_range(30).connect(provider.clone()).await?;
...
-> anyhow::Result<()> {
    ...
    let mut brs = BlockRangeScanner::new().max_block_range(10).connect(provider.clone()).await?;
...
async fn historic_mode_respects_blocks_read_per_epoch() -> anyhow::Result<()> {
    ...
    let mut brs = BlockRangeScanner::new().max_block_range(5).connect(provider.clone()).await?;
    ...
    let mut brs = BlockRangeScanner::new().max_block_range(200).connect(provider).await?;
...
async fn historic_mode_normalises_start_and_end_block() -> anyhow::Result<()> {
    ...
    let mut brs = BlockRangeScanner::new().max_block_range(5).connect(provider).await?;
...
async fn rewind_single_batch_when_epoch_larger_than_range() -> anyhow::Result<()> {
    ...
    let brs = BlockRangeScanner::new().max_block_range(100).connect(provider).await?;
...
async fn rewind_exact_multiple_of_epoch_creates_full_batches_in_reverse() -> anyhow::Result<()> {
    ...
    let brs = BlockRangeScanner::new().max_block_range(5).connect(provider).await?;
...
async fn rewind_with_remainder_trims_first_batch_to_stream_start() -> anyhow::Result<()> {
    ...
    let brs = BlockRangeScanner::new().max_block_range(4).connect(provider).await?;
...
async fn rewind_epoch_of_one_sends_each_block_in_reverse_order() -> anyhow::Result<()> {
    ...
    let brs = BlockRangeScanner::new().max_block_range(1).connect(provider).await?;
...
async fn command_rewind_defaults_latest_to_earliest_batches_correctly() -> anyhow::Result<()> {
    ...
    let brs = BlockRangeScanner::new().max_block_range(7).connect(provider).await?;
...
async fn rewind_reorg_emits_notification_and_rescans_affected_range() -> anyhow::Result<()> {
    ...
    let brs = BlockRangeScanner::new().max_block_range(5).connect(provider.clone()).await?;
