...
async fn live_mode_processes_all_blocks_respecting_block_confirmations() -> anyhow::Result<()> {
    ...
    let mut client = BlockRangeScannerBuilder::new().connect(provider.clone()).await?;
...
async fn stream_from_starts_at_latest_once_it_has_enough_confirmations() -> anyhow::Result<()> {
    ...
    let client = BlockRangeScannerBuilder::new().connect(provider.clone()).await?;
...
async fn historic_mode_respects_blocks_read_per_epoch() -> anyhow::Result<()> {
    ...
    let mut client = BlockRangeScannerBuilder::new().max_block_range(200).connect(provider).await?;
...
async fn historic_mode_normalises_start_and_end_block() -> anyhow::Result<()> {
    ...
    let mut client = BlockRangeScannerBuilder::new().max_block_range(5).connect(provider).await?;
...
async fn rewind_single_batch_when_epoch_larger_than_range() -> anyhow::Result<()> {
    ...
    let mut client = BlockRangeScannerBuilder::new().max_block_range(100).connect(provider).await?;
...
async fn rewind_with_remainder_trims_first_batch_to_stream_start() -> anyhow::Result<()> {
    ...
    let mut client = BlockRangeScannerBuilder::new().max_block_range(4).connect(provider).await?;
...
async fn rewind_epoch_of_one_sends_each_block_in_reverse_order() -> anyhow::Result<()> {
    ...
    let mut client = BlockRangeScannerBuilder::new().max_block_range(1).connect(provider).await?;
...
async fn command_rewind_defaults_latest_to_earliest_batches_correctly() -> anyhow::Result<()> {
    ...
    let mut client = BlockRangeScannerBuilder::new().max_block_range(7).connect(provider).await?;
